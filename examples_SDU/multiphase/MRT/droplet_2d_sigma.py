...
class Droplet2D(MultiphaseMRT):
    ...
    def output_data(self, **kwargs):
        ...
        self.last_p = np.array(kwargs["p"][0, ...])

        rho = self.last_rho
        u = self.last_u
        p = self.last_p

        A_ = np.asarray(self.A).squeeze().item()
        k_ = np.asarray(self.k).squeeze().item()
        kappa_ = np.asarray(self.kappa).squeeze().item()
        ...
        p_l = p[self.nx // 2, self.ny // 2, 0]
        p_g = 0.25 * (p_north + p_south + p_west + p_east)
        pressure_difference = p_l - p_g
        radius = 0.5 * self.droplet_diameter(rho[:,:,0], self.nx // 2, self.ny // 2, rho_l, rho_g)
        sigma_LB = np.abs(pressure_difference*radius)
        interface = self.interface_thickness(rho[:,:,0], self.nx // 2, self.ny // 2, rho_l, rho_g)
        self.last_int_thick = interface
        print(f"SurfTens: {np.abs(pressure_difference*radius)}, interThick: {interface}")
        
        save_fields_vtk(timestep, fields, "output", f"data_{temp:.2f}_{A_:.3f}_{k_:.3f}_{kappa_:.1f}")
        ...
        row = {
            ...
            "rho_l_pred": rho_l,
            "rho_g_pred": rho_g,
            "rho_l_calc": rho_l_pred,
            "rho_g_calc": rho_g_pred,
            "p_l": p_l,
            "p_g": p_g,
            "delta_p": p_l - p_g,
            "radius": radius,
            "sigma": sigma_LB,
            "interface": interface,
            "spurious_current": np.max(np.sqrt(np.sum(u**2, axis=-1)))
        }

        csv_file = "kappa_all_trials.csv"
...
def run_simulation(T_X, kappa_val, k_val, A_val, rho_l_local, rho_g_local, steps=10000): 
    global rho_l, rho_g, temp
    rho_l = rho_l_local
    rho_g = rho_g_local
    temp = T_X
    ...
    return sim.last_rho, sim.last_p
