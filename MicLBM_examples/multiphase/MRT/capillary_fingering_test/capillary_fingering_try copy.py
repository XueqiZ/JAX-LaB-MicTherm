...
class CapillaryFingering(MultiphaseMRT):
    ...
    # Use the parent EOS-based potential for this stability test.
    # To switch back to the simpler force model, uncomment this override.
    # @partial(jit, static_argnums=(0,))
    # def compute_potential(self, rho_tree):
    #     U_tree = map(lambda rho: jnp.zeros_like(rho), rho_tree)
    #     return rho_tree, U_tree
    ...
    def output_data(self, **kwargs):
        ...
        timestep = kwargs["timestep"]
        g_kkprime = np.array(self.g_kkprime)
        sc_interaction_d = g_kkprime[0, 0] * rho_d + g_kkprime[0, 1] * rho_i
        sc_interaction_i = g_kkprime[1, 0] * rho_d + g_kkprime[1, 1] * rho_i
        p_sc_d = rho_d / 3 + 1.5 * rho_d * sc_interaction_d
        p_sc_i = rho_i / 3 + 1.5 * rho_i * sc_interaction_i
        fields = {
            "p": p[..., 0],
            "p_d": p_d[..., 0],
            "p_i": p_i[..., 0],
            "p_sc_total": (p_sc_d + p_sc_i)[..., 0],
            "p_sc_displaced": p_sc_d[..., 0],
            "p_sc_invading": p_sc_i[..., 0],
            "rho": rho[..., 0],
...
if __name__ == "__main__":
    ...
    Tc = 0.5714285714
    T = 0.8 * Tc
    eos = VanderWaal(a=[a*0.8, a], b=[b*0.8, b], R=[R, R], T=T, debugging=debugging == 1)

    g_kkprime = -0.7 * np.ones((2, 2))
    g = 0.1
    g_kkprime[0, 1] = g
    g_kkprime[1, 0] = g
    e = LatticeD2Q9().c.T
